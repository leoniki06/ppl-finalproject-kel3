...
<div class="card" style="padding:18px;">
    ...
    {{-- SEARCH + SORT --}}
    <form method="GET" action="{{ route('seller.products.index') }}" style="display:flex; gap:12px; margin-top:16px; flex-wrap:wrap;">
        <input type="text" name="q" value="{{ request('q') }}" placeholder="Search product..."
               class="ctl" style="flex:1; min-width:220px;">
            ...
            <form method="POST" action="{{ route('seller.products.store') }}" enctype="multipart/form-data">
                ...
                <div style="display:grid; grid-template-columns:1fr 1fr; gap:12px;">
                    ...
                    <div class="fg">
                        ...
                        <input class="ctl" name="category" value="{{ old('category') }}" required placeholder="bakery / drink / snack">
                    </div>

                    <div class="fg">
                        <label>Stock</label>
                        <input class="ctl" name="stock" type="number" min="0" value="{{ old('stock',0) }}" required>
                    </div>

                    <div class="fg">
                        <label>Price</label>
                        <input class="ctl" name="price" type="number" min="0" step="1000" value="{{ old('price') }}" required>
                    </div>

                    <div class="fg">
                        <label>Original Price</label>
                        <input class="ctl" name="original_price" type="number" min="0" step="1000" value="{{ old('original_price') }}" required>
                    </div>

                    <div class="fg">
                        <label>Discount Percent (optional)</label>
                        <input class="ctl" name="discount_percent" type="number" min="0" max="100" value="{{ old('discount_percent',0) }}">
                    </div>

                    <div class="fg">
                        <label>Expiry Date</label>
                        <input class="ctl" name="expiry_date" type="date" value="{{ old('expiry_date') }}" required>
                    </div>
                </div>
...
@push('scripts')
<script>
    // kalau layout.seller kamu belum punya modal helper,
    // ini versi mini supaya modal bisa kebuka/tutup.
    window.LB = window.LB || {};
    LB.open = function(id){
